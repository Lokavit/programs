<template>
  <nav class="nav_container">
    <el-menu :default-active="$route.path" router active-text-color="#FFF" @open="handleOpen" @close="handleClose" :unique-opened="true">
      <template v-for="(item,index) in menus">
        <template v-if="item.meta">
          <el-submenu :index="item.path">
            <template slot="title">
              <i :class="`kft-icon-${item.meta.icon}`"></i>
              <span>{{item.meta.title}}</span>
            </template>
            <template v-for="(child,index) in item.children">
              <template v-if="child.meta">
                <el-menu-item :key="index" :index="child.path" @click="selectItem(child.name,child.meta.title)" style="padding:0 45px">{{child.meta.title}}</el-menu-item>
              </template>
            </template>
          </el-submenu>
        </template>
    ...
    <!-- <div class="ship_left">
      <i class="kft-icon-ship-left" style="font-size:32em;"></i>
    </div>-->
  </nav>
</template>
