<template>
  <section :style="{'width':width}" class="modal_list">
    ...
    <!-- 列表区域 -->
    <el-table v-loading="tableLoading" :data="tableData" border lazy :load="lazyLoad" row-key="entry" :tree-props="{children: 'children', hasChildren: 'hasElement'}" highlight-current-row size="mini" :header-cell-style="headerCellStyle" :cell-style="cellStyle" @current-change="handleCurrentRow" empty-text="请输入项目编号进行查询" :max-height="maxHeight" stripe>
...
export default {
  ...
  computed: {
    ...
    /** 当前表单的所有标注 */
    labels() {
      return this.$store.getters.labels.Zone.listLbales;
    },
    /** 计算 页面table 高度 用于滚动条 */
    maxHeight() {
      /**
  ...
  methods: {
    ...
    async uploadFile(file) {
      ...
      const res = await postZoneImport(formData);
