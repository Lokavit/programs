...
<script>
import { getGoodsTransfer } from "@/api/stock";
import FormGoodsTransfer from "./FormGoodsTransfer";
export default {
  // 库存模块 库存转储单 列表页
  name: "ListGoodsTransfer",
  components: { FormGoodsTransfer },
  data() {
    return {
      /** 列表中的表头样式 */
  ...
  methods: {
    ...
    async getDataList(page, size, search = {}) {
      ...
      try {
        ...
        let res = await getGoodsTransfer(params);
        this.tableData = res;
      } catch (err) {}
      /** 列表数据加载效果：关闭 */
