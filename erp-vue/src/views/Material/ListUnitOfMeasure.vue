<template>
  <section :style="{'width':width}" class="modal_list">
    ...
    <el-table v-loading="tableLoading" :data="tableData.content" border highlight-current-row size="mini" :header-cell-style="headerCellStyle" :cell-style="cellStyle" stripe>
      ...
      <el-table-column prop="uomCode" label="计量单位编码" width="120px" align="center">
        <template v-slot="scope">
          <span @click="onCell" class="span-hover">{{scope.row.uomCode}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="uomName" label="计量单位名称" align="center"></el-table-column>
      ...
      <el-table-column prop="volumn" label="体积" align="center"></el-table-column>
      <el-table-column prop="volumnUnit" label="体积单位" align="center"></el-table-column>
...
export default {
  ...
  computed: {
    ...
    /** 当前窗体标注组 [后台获取所有标注] */
    labels() {
      return this.$store.getters.labels.UOM.listLbales;
    }
  },
  watch: {},
  created() {},
