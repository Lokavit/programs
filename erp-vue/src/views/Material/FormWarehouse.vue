  ...
  <section :style="{'width':width}" v-loading="formLoading">
    <el-form :model="form" size="mini" :rules="rules" ref="form">
      ...
      <div class="form_container">
        <!-- 主表单行单项 之 仓库编码 -->
        <div class="form_row">
          <KftLabel :label="labels.whsCode.value|FormatLabelSuffix" :isRequired="true"></KftLabel>
          <el-form-item prop="whsCode">
            <el-input v-model="form.whsCode" :placeholder="`请输入${labels.whsCode.value}`" :disabled="isDisabled" clearable></el-input>
          </el-form-item>
        </div>
        <!-- 主表单行单项 之 仓库名称 -->
        <div class="form_row">
          <KftLabel :label="labels.whsName.value|FormatLabelSuffix" :isRequired="true"></KftLabel>
          <el-form-item prop="whsName">
            <el-input v-model="form.whsName" :placeholder="`请输入${labels.whsName.value}`" clearable></el-input>
          </el-form-item>
        </div>
      </div>
      <!-- 主表区域  包裹器 end -->
...
<script>
import { getWarehouseInfo, postWarehouse, putWarehouse } from "@/api/stock";
export default {
  // 库存模块 仓库 表单页
  name: "FormWarehouse",
  data() {
  ...
  computed: {
    ...
    /** 当前窗体标注组 [后台获取所有标注] */
    labels() {
      return this.$store.getters.labels.Warehouse.formLabels;
    },
    /** 全局窗体组 */
    modalGroup() {
  ...
  methods: {
    ...
    async getDataInfo() {
      try {
        /** 窗体内容加载效果：开启 */
        this.formLoading = true;
        const res = await getWarehouseInfo(this.passValue);
        this.form = res;
      } catch (error) {}
      /** 窗体内容加载效果：关闭 */
      this.formLoading = false;
    ...
    async putData() {
      try {
        const res = await putWarehouse(this.form);
        this.form = res;
