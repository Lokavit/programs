<template>
  <section :style="{'width':width}" class="modal_list">
    ...
    <el-table v-loading="tableLoading" :data="tableData.content" border highlight-current-row size="mini" :header-cell-style="headerCellStyle" :cell-style="cellStyle" stripe>
      ...
      <el-table-column prop="requiredDate" label="必需日期" show-overflow-tooltip align="center"></el-table-column>
    </el-table>
    <!-- 分页区域 -->
...
<script>
import { getPurchaseOrderRequest } from "@/api/purchase";
import FormPurchaseOrderRequest from "./FormPurchaseOrderRequest";
export default {
  // 采购模块 采购申请单 列表页
  name: "ListPurchaseOrderRequest",
  components: { FormPurchaseOrderRequest },
  data() {
    return {
      /** 列表中的表头样式 */
  ...
  computed: {
    ...
    /** 当前窗体标注组 [后台获取所有标注] */
    labels() {
      return this.$store.getters.labels.PurchaseOrderRequest.listLbales;
    }
  },
  watch: {},
  created() {},
  ...
  methods: {
    ...
    async getDataList(page, size, search = {}) {
      ...
      try {
        ...
        let res = await getPurchaseOrderRequest(params);
        this.tableData = res;
      } catch (err) {}
      /** 列表数据加载效果：关闭 */
    ...
    openModal(code = "") {
      let newModal = {
        ...
        content: {
          component: FormPurchaseOrderRequest,
          code: code
        }
      };
