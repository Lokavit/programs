<template>
  <section :style="{'width':width}" class="modal_list">
    ...
    <el-table v-loading="tableLoading" :data="tableData.content" border highlight-current-row size="mini" :header-cell-style="headerCellStyle" :cell-style="cellStyle" empty-text="请输入项目编码进行查询" stripe>
      ...
      <el-table-column prop="workMethod" label="作业部门" show-overflow-tooltip align="center"></el-table-column>
      <el-table-column prop="requiredDate" label="所需日期" show-overflow-tooltip align="center"></el-table-column>
      <el-table-column prop="workPackageType" label="作业包类型" show-overflow-tooltip align="center"></el-table-column>
...
<script>
import { getWorkPackages } from "@/api/manufacture";
import FormWorkPackage from "./FormWorkPackage";
import { getSelectLists } from "@/api/common";
export default {
  // 生产模块 作业包 列表页
  name: "ListWorkPackage",
  components: { FormWorkPackage },
  data() {
    return {
      /** 列表中的表头样式 */
  ...
  computed: {
    ...
    /** 当前窗体标注组 [后台获取所有标注] */
    labels() {
      return this.$store.getters.labels.WorkPackage.listLbales;
    }
  },
  watch: {},
  created() {},
  ...
  methods: {
    ...
    async getDataList(page, size, search = {}) {
      ...
      try {
        ...
        let res = await getWorkPackages(params);
        this.tableData = res;
      } catch (err) {}
      /** 列表数据加载效果：关闭 */
