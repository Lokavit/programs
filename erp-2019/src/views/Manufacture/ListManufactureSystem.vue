  ...
  <section :style="{'width':width}" class="modal_list">
    ...
    <el-form :inline="true" size="mini">
      <div class="form_row">
        <!-- 第一列 查询输入框  -->
        <!-- <aside>
          <KftLabel :label="`关键词`"></KftLabel>
          <el-input v-model="search.keyword" :placeholder="`请输入关键词`" @keyup.enter.native="onSubmit" clearable @clear="onSubmit" size="mini"></el-input>
        </aside>-->
        <!-- 第二列 查询按钮 -->
        <!-- <aside>
        ...
        <aside style="position:relative">
          <el-button type="info" @click="onRootCreate" size="mini" style="position:absolute;right:70px">
            <i class="kft-icon-create"></i>
            <span>添加</span>
          </el-button>
          <el-button type="info" @click="getSystemData" size="mini" style="position:absolute;right:0px">
...
<script>
import {
  getSystemCommoncodes,
  getSystemCommoncodesInfo,
  postSystemCommoncodes,
  putSystemCommoncodes,
  deleteSystemCommoncodes
} from "@/api/common";
export default {
  // 生产模块 数据字典 列表页
  ...
  computed: {
    ...
    /** 当前窗体标注组 [后台获取所有标注] */
    labels() {
      return this.$store.getters.labels.ManufactureSystem.formLabels;
    },
    /** 计算 页面table 高度 用于滚动条 */
    maxHeight() {
      /**
       * 此处使用本页面求出的width值通过16:9的计算 -160结果为表格最大高度
       * 大屏情况下有问题，待修复
       */
  ...
  methods: {
    // onSubmit() {
    //   console.log("列表一级数据：", this.tableData);
    // },
    ...
    onModifty(row) {
      ...
      // this.getSystemDataInfo(row); // 或者严禁一些，走请求
    },
    ...
    async lazyLoad(row, treeNode, resolve) {
      ...
      const res = await getSystemCommoncodes(params);
      resolve(res); // 子集数据
      /**
       * 把请求回来的数据，直接加在节点上。
    ...
    async getSystemData() {
      try {
        ...
        let res = await getSystemCommoncodes({});
        this.tableData = res; // 最新数据
      } catch (err) {}
      /** 列表数据加载效果：关闭 */
    ...
    async postData() {
      try {
        const res = await postSystemCommoncodes(this.form);
    ...
    async putData() {
      try {
        const res = await putSystemCommoncodes(this.form);
        this.form = res;
        this.getSystemData();
    ...
    async getDataInfo() {
      try {
        /** 窗体内容加载效果：开启 */
        this.formLoading = true;
        const res = await getSystemCommoncodesInfo(entry);
        this.form = res;
      } catch (error) {}
      /** 窗体内容加载效果：关闭 */
      this.formLoading = false;
