<template>
  <section :style="{'width':width}" class="modal_list">
    ...
    <el-table v-loading="tableLoading" :data="tableData.content" border highlight-current-row size="mini" :header-cell-style="headerCellStyle" :cell-style="cellStyle" stripe>
      ...
      <el-table-column prop="entry" label="代码" align="center">
      ...
      <el-table-column prop="membership" label="总人数" show-overflow-tooltip align="center"></el-table-column>
      <el-table-column prop="effectiveMembership" label="操作人数" show-overflow-tooltip align="center"></el-table-column>
      <el-table-column prop="effectivePeriod" label="作业时长" show-overflow-tooltip align="center"></el-table-column>
    </el-table>
    <!-- 分页区域 -->
...
<script>
import { getWorkteams } from "@/api/manufacture";
import FormWorkteam from "./FormWorkteam";
export default {
  // 生产模块 职班 列表页
  name: "ListWorkteam",
  components: { FormWorkteam },
  data() {
    return {
      /** 列表中的表头样式 */
  ...
  computed: {
    ...
    /** 当前窗体标注组 [后台获取所有标注] */
    labels() {
      return this.$store.getters.labels.Workteam.listLbales;
    }
  },
  watch: {},
  created() {},
  ...
  methods: {
    ...
    async getDataList(page, size, search = {}) {
      ...
      try {
        ...
        let res = await getWorkteams(params);
        console.log("RES:", res);
        this.tableData = res;
      } catch (err) {}
      /** 列表数据加载效果：关闭 */
