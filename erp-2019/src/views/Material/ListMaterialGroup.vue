<template>
  <section :style="{'width':width}" class="modal_list">
    ...
    <el-table v-loading="tableLoading" :data="tableData.content" highlight-current-row size="mini" :header-cell-style="headerCellStyle" :cell-style="cellStyle" stripe>
      ...
      <el-table-column prop="itemGroupCode" label="物料组编码">
        <template v-slot="scope">
          <span @click="onCell" class="span-hover">{{scope.row.itemGroupCode}}</span>
        </template>
      </el-table-column>
...
<script>
import { getMaterialGroup } from "@/api/stock";
import FormMaterialGroup from "./FormMaterialGroup";
export default {
  // 库存模块 物料组 列表页
  name: "ListMaterialGroup",
  components: { FormMaterialGroup },
  data() {
    return {
      /** 列表中的表头样式 */
  ...
  computed: {
    ...
    /** 当前窗体标注组 [后台获取所有标注] */
    labels() {
      return this.$store.getters.labels.MaterialGroup.listLbales;
    }
  },
  created() {},
  beforeMount() {},
  ...
  methods: {
    ...
    async getDataList(page, size, search = {}) {
      ...
      try {
        ...
        let res = await getMaterialGroup(params);
        this.tableData = res;
      } catch (err) {}
      /** 列表数据加载效果：关闭 */
