  ...
  <section :style="{'width':width}" v-loading="formLoading">
    <el-form :model="form" size="mini" :rules="rules" ref="form">
      ...
      <el-table :data="tableData" border highlight-current-row size="mini" :header-cell-style="headerCellStyle" :cell-style="cellStyle" :style="[styleObject]" :max-height="maxHeight" :row-style="rowStyle" :row-class-name="rowClassName" @current-change="handleCurrentRow">
        ...
        <el-table-column label="从仓库编码" prop="fromWhsCode">
          <template v-slot="scope">
            <small>
              <KftChoose code="Warehouse" class="kft-line-choose" size="mini" show-detail show-index show-search v-model="scope.row.fromWhsCode" @on-choose-selected="chooseWarehouseFrom($event, scope.$index)" placeholder="请选择仓库"></KftChoose>
            </small>
            <span>{{scope.row.fromWhsCode}}</span>
          </template>
        </el-table-column>
        <el-table-column label="从仓库名称" show-overflow-tooltip prop="fromWhsName"></el-table-column>

        <el-table-column label="到仓库编码" prop="toWhsCode">
          <template v-slot="scope">
            <small>
              <KftChoose code="Warehouse" class="kft-line-choose" size="mini" show-detail show-index show-search v-model="scope.row.toWhsCode" @on-choose-selected="chooseWarehouseTo($event, scope.$index)" placeholder="请选择仓库"></KftChoose>
            </small>
            <span>{{scope.row.toWhsCode}}</span>
          </template>
        </el-table-column>
        <el-table-column label="到仓库名称" show-overflow-tooltip prop="toWhsName"></el-table-column>
    ...
    <template v-if="dialogShow">
      <BatchSelectDialog :goodsList="tData" :dialog-visible="dialogShow" @on-comfirm="confirmHandler" @on-cancel="cancelHandler" :transfer="true"></BatchSelectDialog>
    </template>
...
export default {
  ...
  methods: {
    ...
    chooseMaterial(selectedRow, index) {
      ...
      this.$set(row, "fromWhsCode", selectedRow.defaultWhsCode);
      this.$set(row, "fromWhsName", selectedRow.defaultWhsName);
    },
    /** 从仓库的chooseList 选中  */
    chooseWarehouseFrom(selectedRow, index) {
      let row = this.tableData[index];
      this.$set(row, "fromWhsCode", selectedRow.whsCode);
      this.$set(row, "fromWhsName", selectedRow.whsName);
    },
    /** 到仓库的chooseList 选中  */
    chooseWarehouseTo(selectedRow, index) {
      let row = this.tableData[index];
      this.$set(row, "toWhsCode", selectedRow.whsCode);
      this.$set(row, "toWhsName", selectedRow.whsName);
    },
    ...
    async getDataInfo() {
      try {
        /** 窗体内容加载效果：开启 */
        this.formLoading = true;
        const res = await getGoodsTransferInfo(this.passValue);
        this.form = res;
    ...
    async postData() {
      try {
        ...
        const res = await postGoodsTransfer(this.form);
