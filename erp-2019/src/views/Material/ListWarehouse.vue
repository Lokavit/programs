<template>
  <section :style="{'width':width}" class="modal_list">
    ...
    <!-- 列表区域 -->
    <el-table v-loading="tableLoading" :data="tableData.content" border highlight-current-row size="mini" :header-cell-style="headerCellStyle" :cell-style="cellStyle" stripe @row-dblclick="onSelectRow">
      <el-table-column type="index" align="center"></el-table-column>
      <el-table-column prop="whsCode" label="仓库编码" align="center">
        <template v-slot="scope">
          <span @click="onCell" class="span-hover">{{scope.row.whsCode}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="whsName" label="仓库名称" show-overflow-tooltip align="center"></el-table-column>
    </el-table>
    <!-- 分页区域 -->
...
<script>
import { getWarehouse } from "@/api/stock";
...
export default {
  ...
  computed: {
    ...
    /** 当前窗体标注组 [后台获取所有标注] */
    labels() {
      return this.$store.getters.labels.Warehouse.listLbales;
    }
  },
  watch: {},
  created() {},
  ...
  methods: {
    ...
    createTask(modal) {
      ...
      let newTask = {
        ...
        show: modal.show // modal显隐 统窗体状态。2019.12.26
      };
