  ...
  <section :style="{'width':width}" class="modal_list">
    ...
    <el-form :inline="true" :model="search" size="mini" @submit.native.prevent>
      <div class="form_row" style="margin-bottom:14px;">
        ...
        <aside>
          <KftLabel :label="labels.itemGroupCode.value|FormatLabelSuffix"></KftLabel>
          <KftChoose code="MaterialGroup" v-model="search.itemGroupCode" show-detail @on-choose-selected="chooseMaterialGroup" :placeholder="`请选择${labels.itemGroupCode.value}`" size="mini"></KftChoose>
        </aside>
        <aside>
    ...
    <!-- 列表区域 -->
    <el-table v-loading="tableLoading" :data="tableData" border highlight-current-row size="mini" :header-cell-style="headerCellStyle" :cell-style="cellStyle" :max-height="maxHeight" empty-text="暂无数据" stripe>
      ...
      <el-table-column prop="itemDescription" label="物料描述" show-overflow-tooltip align="center" min-width="16%"></el-table-column>
      ...
      <el-table-column prop="createTime" label="创建时间" show-overflow-tooltip align="center" min-width="18%"></el-table-column>
    </el-table>
  </section>
</template>

<script>
import { getReportJournal } from "@/api/stock";
export default {
  // 库存模块 库存过帐清单 列表页
  ...
  computed: {
    ...
    /** 当前窗体标注组 [后台获取所有标注] */
    labels() {
      return this.$store.getters.labels.GoodsReportJournal.listLbales;
    },
    /** 计算table 高度 */
    maxHeight() {
      /**
  ...
  methods: {
    ...
    chooseMaterialGroup(selectedRow) {
      this.$set(this.search, "itemGroupCode", selectedRow.itemGroupCode);
    },
    /** 仓库的chooseList 选中 */
    ...
    async getDataList(params) {
      ...
      try {
        ...
        let res = await getReportJournal(params);
