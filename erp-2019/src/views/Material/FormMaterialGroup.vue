      ...
      <div class="form_container">
        ...
        <div class="form_row">
          ...
          <el-form-item prop="itemGroupCode">
            <el-input v-model="form.itemGroupCode" :placeholder="`请输入${labels.itemGroupCode.value}`" :disabled="isDisabled" clearable></el-input>
          </el-form-item>
        </div>
        <!-- 主表单行单项 之 物料组名称 -->
        <div class="form_row">
          <KftLabel :label="labels.itemGroupName.value|FormatLabelSuffix" :isRequired="true"></KftLabel>
          <el-form-item prop="itemGroupName">
            <el-input v-model="form.itemGroupName" :placeholder="`请输入${labels.itemGroupName.value}`" clearable></el-input>
          </el-form-item>
        </div>
      </div>
      <!-- 主表区域  包裹器 end -->
...
export default {
  ...
  computed: {
    ...
    /** 当前窗体标注组 [后台获取所有标注] */
    labels() {
      return this.$store.getters.labels.MaterialGroup.formLabels;
    },
    /** 全局窗体组 */
    modalGroup() {
  ...
  methods: {
    ...
    async getDataInfo() {
      try {
        /** 窗体内容加载效果：开启 */
        this.formLoading = true;
        const res = await getMaterialGroupInfo(this.passValue);
        this.form = res;
      } catch (error) {}
      /** 窗体内容加载效果：关闭 */
      this.formLoading = false;
    ...
    async postData() {
      try {
        const res = await postMaterialGroup(this.form);
    ...
    async putData() {
      try {
        const res = await putMaterialGroup(this.form);
        this.form = res;
