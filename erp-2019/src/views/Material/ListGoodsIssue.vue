<template>
  <section :style="{'width':width}" class="modal_list">
    ...
    <el-form :inline="true" :model="search" size="mini" @submit.native.prevent>
      <div class="form_row">
        ...
        <aside style="min-width:240px;">
          <KftLabel :label="labels.docEntry.value|FormatLabelSuffix" style="margin-left:20px;"></KftLabel>
          <el-input v-model="search.docEntry" :placeholder="`请输入${labels.docEntry.value}`" @keyup.enter.native="onSubmit" clearable @clear="onSubmit" size="mini"></el-input>
        </aside>
        <!-- 第二列 查询按钮 -->
        <aside>
    ...
    <el-table v-loading="tableLoading" :data="tableData.content" border highlight-current-row size="mini" :header-cell-style="headerCellStyle" :cell-style="cellStyle" stripe>
      ...
      <el-table-column prop="docEntry" label="单据编号" align="center" min-width="20%">
        <template v-slot="scope">
          <span @click="onCell" class="span-hover">{{scope.row.docEntry}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="docDate" label="单据日期" show-overflow-tooltip align="center" min-width="40%"></el-table-column>
      <el-table-column prop="createTime" label="创建时间" show-overflow-tooltip align="center" min-width="40%"></el-table-column>
    </el-table>
    <!-- 分页区域 -->
...
<script>
import { getGoodsIssue } from "@/api/stock";
import FormGoodsIssue from "./FormGoodsIssue";
export default {
  // 库存模块 库存发货单 列表页
  name: "ListGoodsIssue",
  components: { FormGoodsIssue },
  data() {
    return {
      /** 列表中的表头样式 */
  ...
  computed: {
    ...
    /** 当前窗体标注组 [后台获取所有标注] */
    labels() {
      return this.$store.getters.labels.Document.listLbales;
    }
  },
  watch: {},
  created() {},
  ...
  methods: {
    ...
    async getDataList(page, size, search = {}) {
      ...
      try {
        ...
        let res = await getGoodsIssue(params);
        this.tableData = res;
      } catch (err) {}
      /** 列表数据加载效果：关闭 */
