<template>
  <section :style="{'width':width}" class="modal_list">
    ...
    <el-table v-loading="tableLoading" :data="tableData.content" border highlight-current-row size="mini" :header-cell-style="headerCellStyle" :cell-style="cellStyle" stripe>
      ...
      <el-table-column prop="docEntry" label="单据编号" align="center">
      ...
      <el-table-column prop="vendorName" label="供应商名称" align="center"></el-table-column>
    </el-table>
    <!-- 分页区域 -->
...
<script>
import { getPurchaseDelivery } from "@/api/purchase";
import FormPurchaseDelivery from "./FormPurchaseDelivery";
export default {
  // 采购模块 采购收货单 列表页
  name: "ListPurchaseDelivery",
  components: { FormPurchaseDelivery },
  data() {
    return {
      /** 列表中的表头样式 */
  ...
  computed: {
    ...
    /** 当前窗体标注组 [后台获取所有标注] */
    labels() {
      return this.$store.getters.labels.PurchaseDelivery.listLbales;
    }
  },
  watch: {},
  created() {},
  ...
  methods: {
    ...
    async getDataList(page, size, search = {}) {
      ...
      try {
        ...
        let res = await getPurchaseDelivery(params);
        this.tableData = res;
      } catch (err) {}
      /** 列表数据加载效果：关闭 */
    ...
    openModal(code = "") {
      let newModal = {
        ...
        content: {
          component: FormPurchaseDelivery,
          code: code
        }
      };
