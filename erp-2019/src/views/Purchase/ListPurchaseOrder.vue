<template>
  <section :style="{'width':width}" class="modal_list">
    ...
    <el-table v-loading="tableLoading" :data="tableData.content" border highlight-current-row size="mini" :header-cell-style="headerCellStyle" :cell-style="cellStyle" stripe>
      ...
      <el-table-column prop="docEntry" label="单据编号" width="120px" align="center">
        <template v-slot="scope">
          <span @click="onCell" class="span-hover">{{scope.row.docEntry}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="docDate" label="单据日期" show-overflow-tooltip align="center"></el-table-column>
      <el-table-column prop="docDueDate" label="到期日" show-overflow-tooltip align="center"></el-table-column>
      <el-table-column prop="deliveryDate" label="交货日期" show-overflow-tooltip align="center"></el-table-column>
      <el-table-column prop="purchaserCode" label="采购员编码" align="center"></el-table-column>
      <el-table-column prop="purchaserName" label="采购员姓名" align="center"></el-table-column>
      <el-table-column prop="vendorCode" label="供应商编码" align="center"></el-table-column>
      <el-table-column prop="vendorName" label="供应商名称" show-overflow-tooltip align="center"></el-table-column>
    </el-table>
    <!-- 分页区域 -->
...
export default {
  ...
  computed: {
    ...
    /** 当前窗体标注组 [后台获取所有标注] */
    labels() {
      return this.$store.getters.labels.PurchaseOrder.listLbales;
    }
  },
  watch: {},
  created() {},
  ...
  methods: {
     ...
     */
    onSubmit() {
      if (!this.search.docEntry) this.getDataList();
      if (this.search.docEntry) {
    ...
    async getDataList(page, size, search = {}) {
      ...
      try {
        ...
        let res = await getPurchaseOrder(params);
        this.tableData = res;
      } catch (err) {}
      /** 列表数据加载效果：关闭 */
