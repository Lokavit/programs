<template>
  <section :style="{'width':width}" v-loading="formLoading">
    <el-form :model="form" size="mini" :rules="rules" ref="form">
      ...
      <div class="form_container">
        ...
        <div class="form_row">
          ...
          <!-- 单据日期 -->
          <aside>
            <KftLabel :label="labels.docDate.value|FormatLabelSuffix"></KftLabel>
          ...
          <!-- 到期日 -->
          <aside>
            <KftLabel :label="labels.docDueDate.value|FormatLabelSuffix"></KftLabel>
          ...
          <!-- 供应商编码 -->
          <aside>
            <KftLabel :label="labels.vendorCode.value|FormatLabelSuffix"></KftLabel>
        ...
        <div class="form_row">
          ...
          <!-- 采购员编码 -->
          <aside>
            <KftLabel :label="labels.purchaserCode.value|FormatLabelSuffix"></KftLabel>
      ...
      <el-table :data="tableData" border highlight-current-row size="mini" show-summary :summary-method="getSummaries" :header-cell-style="headerCellStyle" :cell-style="cellStyle" :style="[styleObject]" :max-height="maxHeight" :row-style="rowStyle" :row-class-name="rowClassName" @current-change="handleCurrentRow">
        ...
        <el-table-column label="物料编码" width="140" prop="itemCode">
        ...
        <el-table-column label="物料描述" show-overflow-tooltip prop="itemDescription" width="100"></el-table-column>
        <el-table-column label="仓库编码" width="140" prop="whsCode">
          <template v-slot="scope">
            <small>
        ...
        <el-table-column label="仓库名称" width="80" show-overflow-tooltip prop="whsName"></el-table-column>

        <el-table-column label="数量" width="80" prop="quantity">
          <template v-slot="scope">
            <small>
              <el-input-number size="mini" v-model="scope.row.quantity" :min="1" :max="scope.row.maxQuantity" style="width:100px;"></el-input-number>
            </small>
        ...
        <el-table-column label="价格货币" prop="priceCurrency">
          <template v-slot="scope">
            <small>
        ...
        <el-table-column label="货币汇率" prop="currencyRate">
          <template v-slot="scope">
            <small>
        ...
        <el-table-column label="行总计" prop="lineTotal">
    ...
    <template v-if="copyFromDialogShow">
      <CopyFromDialog type="FormPurchaseReturn" :dialog-visible="copyFromDialogShow" @on-comfirm="confirmHandlerCopyFrom" @on-cancel="cancelHandlerCopyFrom"></CopyFromDialog>
    </template>
...
export default {
  ...
  computed: {
    ...
    /** 当前窗体标注组 [后台获取所有标注] */
    labels() {
      return this.$store.getters.labels.PurchaseReturn.formLabels;
    },
    /** 全局窗体组 */
    modalGroup() {
  ...
  methods: {
    ...
    onSubmit() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          this.form.purchaseReturnLines = this.removeEmptyData(this.tableData);
          /** 用 [version] 版本号来判定，执行新增函数 [该表单无更新函数] */
          if (this.form.version !== undefined) return;
          // 如果表单行数据没有，提醒用户选择
          if (
            this.form.purchaseReturnLines == undefined ||
            this.form.purchaseReturnLines.length <= 0
          ) {
            this.$message.warning("请选择物料");
            return;
          } else {
            // 返回其中 [物料管理方式]为[批次]的数据组
            let needBatchLines = this.form.purchaseReturnLines.filter(
    ...
    async getDataInfo() {
      try {
        /** 窗体内容加载效果：开启 */
        this.formLoading = true;
        const res = await getPurchaseReturnsInfo(this.passValue);
        this.form = res;
        this.tableData = res.purchaseReturnLines;
      } catch (error) {}
      /** 窗体内容加载效果：关闭 */
      this.formLoading = false;
    },
    ...
    async postData() {
      try {
        ...
        for (let i = 0; i < this.form.purchaseReturnLines.length; i++) {
          // 删除对象中的某个属性， {需要删除的属性, ...其他属性的临时变量} = 当前对象
          let { batchCount, ...formData } = this.form.purchaseReturnLines[i];
          this.form.purchaseReturnLines[i] = formData;
        }
        let { purchaseReturnLines, ...otherFormData } = this.form;
        this.form = {
          purchaseReturnLines: this.form.purchaseReturnLines,
          ...otherFormData
        };

        const res = await postPurchaseReturns(this.form);
        this.form = res; // 返回结果中的数据，回显至表单上
        this.$message.success("提交成功");
        this.form.purchaseReturnLines = res.purchaseReturnLines; // 提交成功之后的回显 内嵌行
        /** 隐藏操作列 设置禁输入 */
        this.isOperate = false;
        this.currentRow = []; // 清掉当前行
        this.tableData = this.form.purchaseReturnLines;
        // 内嵌行中的数据，移除空数据行
    ...
    async putData() {
      try {
        const res = await putPurchaseReturns(this.form);
        this.form = res;
