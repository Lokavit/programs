...
class SoundBlocks {
  ...
  _getSoundState(target) {
    let soundState = target.getCustomState(SoundBlocks.STATE_KEY);
    if (!soundState) {
      soundState = Utility.cloneSimple(SoundBlocks.DEFAULT_SOUND_STATE);
      target.setCustomState(SoundBlocks.STATE_KEY, soundState);
  ...
  _onTargetCreated(newTarget, sourceTarget) {
    if (sourceTarget) {
      const soundState = sourceTarget.getCustomState(SoundBlocks.STATE_KEY);
      if (soundState && newTarget) {
        newTarget.setCustomState(
          SoundBlocks.STATE_KEY,
  ...
  getPrimitives() {
    return {
      pro_sound_play: this.playSound,
      pro_sound_playuntildone: this.playSoundAndWait,
      pro_sound_stopallsounds: this.stopAllSounds,
      pro_sound_seteffectto: this.setEffect,
      pro_sound_changeeffectby: this.changeEffect,
      pro_sound_cleareffects: this.clearEffects,
      pro_sound_sounds_menu: this.soundsMenu,
      pro_sound_beats_menu: this.beatsMenu,
      pro_sound_effects_menu: this.effectsMenu,
      pro_sound_setvolumeto: this.setVolume,
      pro_sound_changevolumeby: this.changeVolume,
      pro_sound_volume: this.getVolume,
    };
  }
  ...
  _updateEffect(args, util, change) {
    ...
    const { min, max } = SoundBlocks.EFFECT_RANGE[effect];
