/*
 * @Author: Satya
 * @Date: 2020-07-21 13:43:50
 * @Last Modified by: Satya
 * @Last Modified time: 2020-11-27 16:00:25
 * doc:外观类积木块命令
 */
...
class LooksBlocks {
  constructor(runtime) {
    ...
    this.runtime.on(LooksBlocks.SAY_OR_THINK, this._updateBubble);
  }
  ...
  _getBubbleState(target) {
    let bubbleState = target.getCustomState(LooksBlocks.STATE_KEY);
    if (!bubbleState) {
      bubbleState = Utility.cloneSimple(LooksBlocks.DEFAULT_BUBBLE_STATE);
      target.setCustomState(LooksBlocks.STATE_KEY, bubbleState);
    }
    return bubbleState;
  }
  ...
  _formatBubbleText(text) {
    ...
    text = String(text).substr(0, LooksBlocks.SAY_BUBBLE_LIMIT);
  ...
  getPrimitives() {
    return {
      ...
      /** KID-PRO 指令 实现 */
      pro_looks_say: this.say,
      pro_looks_sayforsecs: this.sayforsecs,
      pro_looks_think: this.think,
      pro_looks_thinkforsecs: this.thinkforsecs,
      pro_looks_show: this.show,
      pro_looks_hide: this.hide,
      looks_hideallsprites: () => {}, // legacy no-op block
      pro_looks_switchcostumeto: this.switchCostume,
      pro_looks_switchbackdropto: this.switchBackdrop,
      pro_looks_switchbackdroptoandwait: this.switchBackdropAndWait,
      pro_looks_nextcostume: this.nextCostume,
      pro_looks_nextbackdrop: this.nextBackdrop,
      pro_looks_changeeffectby: this.changeEffect,
      pro_looks_seteffectto: this.setEffect,
      pro_looks_cleargraphiceffects: this.clearEffects,
      pro_looks_changesizeby: this.changeSize,
      pro_looks_setsizeto: this.setSize,
      looks_changestretchby: () => {}, // legacy no-op blocks
      looks_setstretchto: () => {},
      pro_looks_gotofrontback: this.goToFrontBack,
      pro_looks_goforwardbackwardlayers: this.goForwardBackwardLayers,
      pro_looks_size: this.getSize,
      pro_looks_costumenumbername: this.getCostumeNumberName,
      pro_looks_backdropnumbername: this.getBackdropNumberName,
    };
  }
    ...
    switch (effect) {
      case "ghost":
        clampedValue = Utility.clamp(
          value,
          LooksBlocks.EFFECT_GHOST_LIMIT.min,
          LooksBlocks.EFFECT_GHOST_LIMIT.max
        );
        break;
      case "brightness":
        clampedValue = Utility.clamp(
          value,
          LooksBlocks.EFFECT_BRIGHTNESS_LIMIT.min,
          LooksBlocks.EFFECT_BRIGHTNESS_LIMIT.max
        );
        break;
    }
    return clampedValue;
