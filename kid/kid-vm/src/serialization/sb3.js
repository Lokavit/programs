...
const serializeTarget = function (target, extensions) {
  ...
  if (
    ...
    target.currentCostume = Utility.clamp(
...
const parseKidAssets = function (object, runtime, zip) {
  console.log("sb3.js 进入 parseKidAssets函数", object, runtime, zip);
  // 观察者/监视器-跳过此对象，直到在VM中实现.
  ...
  assets.costumePromises = (object.costumes || []).map((costumeSource) => {
    ...
    // 应该在上面创建的造型对象而不是源造型对象上调用deserializeCostume，造型对象转换为sb3格式
...
const parseKidObject = function (object, runtime, extensions, zip, assets) {
  ...
  // @todo: 现在，将所有Kid对象（舞台/精灵）加载为Sprite.
  const sprite = new Sprite(blocks, runtime);
  console.log("vm sb3.js 实例化Sprite", sprite);
...
const deserialize = function (json, runtime, zip, isSingleSprite) {
  ...
  return (
    Promise.resolve(
      targetObjects.map((target) => parseKidAssets(target, runtime, zip))
    )
      // 强制此承诺等待js刻度中的下一个循环. 让存储空间有时间发送资产请求.
      .then((assets) => Promise.resolve(assets))
      .then((assets) =>
        Promise.all(
          targetObjects.map((target, index) =>
            parseKidObject(target, runtime, extensions, zip, assets[index])
          )
        )
      )
      .then((targets) =>
        targets // 将目标重新排序为原始的子画面窗格顺序
          .map((t, i) => {
            // 将层顺序属性添加到反序列化的目标.
