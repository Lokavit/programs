...
class Scratch3Speech2TextBlocks {
  ...
  _shouldKeepResult(fuzzyMatchIndex, result, normalizedTranscript) {
    ...
    if (shouldKeepFuzzyMatch) {
      console.info(`Fuzzy match with high stability.`);
      console.info(`match index is  ${fuzzyMatchIndex}`);
      const phrases = this._phraseList.join(" ");
      const matchPhrase = phrases.substring(
        fuzzyMatchIndex,
        fuzzyMatchIndex + normalizedTranscript.length
      );
      console.info(`fuzzy match: ${matchPhrase} in ${normalizedTranscript}`);
    }
  ...
  _processTranscriptionResult(result) {
    ...
    console.info(`Keeing result: ${this._currentUtterance}`);
  ...
  _onTranscriptionFromServer(e) {
    ...
    try {
      ...
      console.error(`Problem parsing json. continuing: ${ex}`);
  ...
  _newWebsocket() {
    ...
    Promise.all([this._audioPromise, websocketPromise])
      ...
      .catch((e) => {
        console.error(`Problem with setup:  ${e}`);
      });
  }
  ...
  _setupSocketCallback(values) {
    ...
    this._socket.addEventListener("error", (e) => {
      console.error(`Error from web socket: ${e}`);
    });
