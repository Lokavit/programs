  ...
  _turnOn() {
    const cmd = this._parent.generateOutputCommand(
      ...
      [
        ...
        Utility.clamp(this.power + BoostMotorMaxPowerAdd, 0, 100),
...
class Boost {
  ...
  send(uuid, message, useLimiter = true) {
    ...
    return this._ble.write(
      BoostBLE.service,
      uuid,
      Utility.uint8ArrayToBase64(message),
      "base64"
    );
  }
...
class Scratch3BoostBlocks {
  ...
  motorOnForRotation(args) {
    // TODO: cast args.MOTOR_ID?
    let degrees = Utility.toNumber(args.ROTATION) * 360;
  ...
  setLightHue(args) {
    ...
    const rgbObject = Utility.hsvToRgb({ h: hue, s: 1, v: 1 });
