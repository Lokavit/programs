 ...
 * call block. 创建XML来表示过程调用块的（不可编辑）名称和参数
...
Blockly.ScratchBlocks.ProcedureUtils.disconnectOldBlocks_ = function () {
  ...
  for (var i = 0, input; (input = this.inputList[i]); i++) {
    if (input.connection) {
      ...
      // the input. 删除影子DOM，然后断开该积木。 否则，阴影块将立即重生，并且在删除输入时必须将其删除
...
/**
 * Remove all inputs on the block, including dummy inputs. 删除块上的所有输入，包括虚拟输入
 * Assumes no input has shadow DOM set. 假设没有输入设置了影子DOM
 * @private
 * @this Blockly.Block
 */
Blockly.ScratchBlocks.ProcedureUtils.removeAllInputs_ = function () {
  // Delete inputs directly instead of with block.removeInput to avoid splicing
  // out of the input list at every index.直接删除输入
  for (var i = 0, input; (input = this.inputList[i]); i++) {
    input.dispose(); // 显示释放资源？
  }
  this.inputList = [];
};
 ...
 * shadow blocks or reconnected old blocks as appropriate. 创建新procCode指定的所有输入，并在适当时用阴影块或重新连接的旧块填充它们
 * @param {!Object.<string, {shadow: Element, block: Blockly.Block}>}
 *     connectionMap An object mapping argument IDs to blocks and shadow DOMs.连接映射对象将参数ID映射到块和影子DOM
 * @private
 * @this Blockly.Block
 */
Blockly.ScratchBlocks.ProcedureUtils.createAllInputs_ = function (
  ...
  // Split the proc into components将proc分成多个部分, by %n(数字), %b(布尔), %s(字符串) (ignoring escaped).
  var procComponents = this.procCode_.split(/(?=[^\\]%[nbs])/);
  procComponents = procComponents.map(function (c) {
    return c.trim(); // Strip whitespace. 带空格
  });
  // Create arguments and labels as appropriate. 根据需要创建参数和标签
  var argumentCount = 0;
...
/**
 * Delete all shadow blocks in the given map. 删除给定贴图中的所有阴影块
 ...
 * procedures_prototype block. 将带有给定文本的标签字段添加到过程调用或过程原型块
...
/**
 * Create a new shadow block and attach it to the given input.创建一个新的阴影块并将其附加到给定的输入
...
/**
 * Create a new argument reporter block. 创建一个新的参数报告程序块
 * @param {string} argumentType One of 'b' (boolean), 's' (string) or
 *     'n' (number).
 ...
 * given input. 通过将正确的子块或阴影附加到给定的输入来填充参数
...
Blockly.ScratchBlocks.ProcedureUtils.populateArgumentOnPrototype_ = function (
  ...
  var oldTypeMatches = Blockly.ScratchBlocks.ProcedureUtils.checkOldTypeMatches_(
    oldBlock,
    type
  );
...
/**
 * Create an argument editor. 创建参数编辑器
 ...
 * and their text. 根据现有输入及其文本更新块上的可序列化信息
 */
...
Blockly.ScratchBlocks.ProcedureUtils.focusLastEditor_ = function () {
  if (this.inputList.length > 0) {
    ...
    if (newInput.type == Blockly.DUMMY_INPUT) {
      ...
      // Inspect the argument editor. 检查参数编辑器
...
/**
 * Callback to remove a field, only for the declaration block. 回调以删除字段，仅用于声明块
 * @param {Blockly.Field} field The field being removed.
 * @public
 */
Blockly.ScratchBlocks.ProcedureUtils.removeFieldCallback = function (field) {
  // Do not delete if there is only one input 如果只有一个输入，请勿删除
