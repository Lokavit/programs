...
# 将核心Blockly文件压缩为单个JavaScript文件.

# This script generates two versions of Blockly's core files:
#   blockly_compressed.js
#   blockly_uncompressed.js

# 压缩文件是通过Google Closure Compiler运行的Blockly所有核心文件的串联。
# 这是使用在线API完成的（需要花费几秒钟，并且需要Internet连接）
# 未压缩文件是一个脚本，该脚本一个接一个地加载到Blockly的每个核心文件中
...
"""))
    ...
    add_dependency = '\n'.join(add_dependency)
    # 找到Blockly目录名称，并将其替换为JS变量.
    # 这样，即使目录名不同，也可以在一台计算机上编译blockly_uncompressed.js，并在另一台计算机上使用.
...
class Gen_compressed(threading.Thread):
  ...
  def gen_blocks(self, block_type):
    ...
    # 读入所有源文件. 添加Blockly.Blocks以与编译器兼容.
    params.append(("js_file", os.path.join("build", "gen_blocks.js")))
    # 添加Blockly.Colours以使用集中式颜色库
