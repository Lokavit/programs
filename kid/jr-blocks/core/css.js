/*
 * @Author: Satya
 * @Date: 2020-05-23 14:10:54
 * @Last Modified by: Satya
 * @Last Modified time: 2020-06-16 14:14:19
 * doc:同步注入Blockly的CSS
 *
 * 为每个分类div设置形状及内部自定义
 */
...
/**
 * 将CSS注入DOM。 这比使用常规CSS文件更好，因为:
 * a) 它是同步加载的，以后不会强制重画.
 * b) 通过不阻塞单独的HTTP传输来加快加载速度.
 * c) 根据init选项，可以使CSS内容动态化.
 * @param {boolean} hasCss 如果为false，则不要注入CSS.
 * @param {string} pathToMedia 页面到Blockly媒体目录的路径.
 */
Blockly.Css.inject = function (hasCss, pathToMedia) {
  // 只注入一次CSS.
  if (Blockly.Css.styleSheet_) return;
  // 游标规则的占位符。 必须是第一个规则（索引0）.
...
Blockly.Css.CONTENT = [
  ...
  */
  ...
  "width: 2rem;", // div的宽度
  "height: 2rem;", // div的高度
  "border: 1px solid;", // 外边框，颜色由xml结构中设置
  "border-radius: 100%;", // 圆角
  "margin: 0 auto 0.125rem;", // div的外边距
  "}",
  // 每个分类div中，包含icon的样式设置
  ".scratchCategoryItemIcon {",
  "width: 3rem;", // 背景图的宽度
  "height: 3rem;", // 背景图的高度
  "margin: 0 auto 0.125rem;", // 外边距，注意距左值
  // "background-size: 100%;",
  "background-size: contain;", // 背景图放大，适应内容区域
