/*
 * @Author: Satya
 * @Date: 2020-06-15 13:50:57
 * @Last Modified by:   Satya
 * @Last Modified time: 2020-06-15 13:50:57
 * doc:以图形方式将块渲染为SVG的方法
 */
 ...
 * @param {?string} prototypeName 每个积木的属性名(如：motion_movesteps)
 * @param {string=} opt_id 可选的ID。如果提供，请使用此ID，否则请创建一个新ID。如果该ID与使用中ID冲突，则会生成一个新ID
 * @extends {Blockly.Block}
 * @constructor
 */
Blockly.BlockSvg = function (workspace, prototypeName, opt_id) {
  // console.log("workspace:", workspace);
  // console.log("opt_id:", opt_id); // 同属性名
  /**
   * @type {SVGElement}
   * @private
   *  创建 svg组元素
  ...
  if (this.svgGroup_.dataset) {
    ...
    // console.log("svgGroup_.dataset：", this.svgGroup_.dataset);
  }
  // 即整个积木块的class、stroke、fill、fill-opacity、d路径值
  // console.log("svg路径：", this.svgPath_);
  // 每个积木块的整组元素：底块、明文、输入区域、输入内容、
  // console.log("svg路径 svgGroup_：", this.svgGroup_);
  // if (this.svgGroup_.dataset.id == "math_number") {
  //   // console.log(this.svgGroup_.dataset.id);
  //   // console.log("找到transform：", this.svgGroup_);
  // }
};
 ...
 * 创建并初始化该块的SVG表示形式
 * May be called more than once. 可能被多次调用
 */
...
Blockly.BlockSvg.prototype.setGlowStack = function (isGlowingStack) {
  ...
  // Update the applied SVG filter if the property has changed 如果属性已更改，请更新应用的SVG过滤器
...
Blockly.BlockSvg.prototype.getRelativeToSurfaceXY = function () {
  ...
  if (element) {
    do {
      // Loop through this block and every parent. 遍历此块和每个父对象
      var xy = Blockly.utils.getRelativeXY(element);
      x += xy.x;
      y += xy.y;
      // If this element is the current element on the drag surface, include
      // the translation of the drag surface itself. 如果此元素是拖动表面上的当前元素，则包括拖动表面本身的平移
      if (
        this.useDragSurface_ &&
...
/**
 * Move a block by a relative offset. 按相对偏移量移动块
 ...
 * 将此块移动到可见工作区的前面。 <g>标签不遵守z-index，因此SVG会按照它们在DOM中的顺序来渲染它们。 通过首先将该块放置在块组的<g>内，它将在任何其他块之上呈现。
 * @package
 */
...
/**
 * Set whether this block returns a value. 设置此块是否返回值
...
/**
 * Remove an input from this block. 从该块中删除输入
...
Blockly.BlockSvg.prototype.removeInput = function (name, opt_quiet) {
  ...
  if (this.rendered) {
    this.render();
    // Removing an input will cause the block to change shape.移除输入将导致块改变形状
    this.bumpNeighbours_();
  }
};
...
/**
 * Returns connections originating from this block. 返回源自此块的连接
...
/**
 * Create a connection of the specified type. 创建指定类型的连接
 ...
 * connected should not coincidentally line up on screen. 未实际关联的积木块，不在屏幕对齐
 * @private
 */
Blockly.BlockSvg.prototype.bumpNeighbours_ = function () {
  if (!this.workspace) {
    return; // Deleted block. 删除积木
  }
  if (this.workspace.isDragging()) {
    return; // Don't bump blocks during a drag. 拖动时不要撞块
  }
  var rootBlock = this.getRootBlock();
  if (rootBlock.isInFlyout) {
    return; // Don't move blocks around in a flyout. 不要在弹出栏中移动块
  }
  // Loop through every connection on this block. 遍历此块上的每个连接
  var myConnections = this.getConnections_(false);
...
/**
 * 将分类下某一积木块拖拽至逻辑区，鼠标释放
 * 计划在短暂的延迟后突然捕捉到网格并碰撞邻居
 * @package
 */
Blockly.BlockSvg.prototype.scheduleSnapAndBump = function () {
  var block = this; // this即当前鼠标上的积木块
  // console.log(
  //   "block_svg.js 计划在短暂的延迟后突然捕捉到网格并碰撞邻居 :",
  //   block
  // );
