/*
 * @Author: Satya
 * @Date: 2020-06-13 11:46:31
 * @Last Modified by: Satya
 * @Last Modified time: 2020-06-13 12:10:22
 * doc:块之间创建连接类
 */
...
/**
 * 块之间创建连接类.
 * @param {!Blockly.Block} source 建立此连接的块.
 * @param {number} type 连接类型.(数字表示,对应值在constants.js中定义)
...
Blockly.RenderedConnection = function (source, type) {
  ...
  /**
   * 工作区单位（0，0）在块的左上方.
   * @type {!goog.math.Coordinate} 表示坐标和位置的类
   * @private
   */
  this.offsetInBlock_ = new goog.math.Coordinate(0, 0);
};
/** google closure框架 closure继承模块的API
 * goog.inherits(childCtor,parentCtor)
 */
goog.inherits(Blockly.RenderedConnection, Blockly.Connection);

/**
 * 返回两个连接之间的距离.(似乎是积木块之间的连接)
 * @param {!Blockly.Connection} otherConnection 另一个连接以测量到的距离.
 * @return {number} 连接之间的距离，以工作空间单位.
 */
