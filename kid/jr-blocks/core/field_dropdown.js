...
/**
 * 在渲染时将下拉箭头放置在适当的位置.
 * @param {number} x 箭头的X位置，以px为单位.
 * @return {number} 箭头占用的空间量（以像素为单位）.
 */
Blockly.FieldDropdown.prototype.positionArrow = function (x) {
  ...
  // 凸块定位在右侧以形成盒式下拉菜单
  if (this.box_) this.arrowX_ += Blockly.BlockSvg.BOX_FIELD_PADDING;

  /**
   * @author satya
   * @description 将此处Y轴值增加18,使其dropdown的下拉箭头与文字水平居中
   */
  this.arrow_.setAttribute(
    "transform",
    "translate(" + this.arrowX_ + "," + (this.arrowY_ + 18) + ")"
  );
  return addedWidth;
};
