/*
 * @Author: Satya
 * @Date: 2020-05-23 10:43:04
 * @Last Modified by: Satya
 * @Last Modified time: 2020-06-11 13:05:20
 * doc:
 *  725行： 该函数内，用于自定义分类图标的呈现效果
 */
...
/**
 * 设置选定项目工厂。包装函数，用于从触摸处理程序调用 setSelectedItem .
 * @param {Blockly.Toolbox.Category} item 选择的分类.
 * @return {function} 可以传递给bindEvent的函数.
 */
 ...
 * 给一个分类创建DOM(原来的蓝色圆形)
 * 想要自定义，即更改此函数逻辑.
 * 如果是需要根据不同分类设置不同图标样式，即对this.id或this.name加判断
 */
Blockly.Toolbox.Category.prototype.createDom = function () {
  // 此处this，每个分类。也就是圆形图标。
  // console.log("给一个分类创建DOM:", this);
  ...
  // );
  /**
   *  如果有iconURI，即在 blocks_vertical\default_toolbox.js文件中加入以下字符串
   * '<category iconURI="../media/move-left.svg">' 即，为该分类图标设置.svg文件
   * 根据是否有iconURI为该分类设置不同的class样式
   */
  if (this.iconURI_) {
    // bubble 即 分类的圆圈
  ...
  // this.item_.appendChild(this.label_); // 移除每个分类下的明文
