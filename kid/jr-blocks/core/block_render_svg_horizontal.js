...
Blockly.BlockSvg.prototype.updateColour = function () {
  var fillColour = this.isGlowing_
  ...
  // Render block fill
  var fillColour = this.isGlowingBlock_
    ? this.getColourSecondary()
    : this.getColour();
...
Blockly.BlockSvg.prototype.renderCompute_ = function () {
  ...
  if (metrics.iconMenu) {
    var currentSrc = metrics.iconMenu.getSrcForValue(
      metrics.iconMenu.getValue()
    );
    if (currentSrc) {
  ...
  // Normal block sizing
  metrics.width =
    Blockly.BlockSvg.SEP_SPACE_X * 2 + Blockly.BlockSvg.IMAGE_FIELD_WIDTH;
  metrics.height =
    Blockly.BlockSvg.SEP_SPACE_Y * 2 + Blockly.BlockSvg.IMAGE_FIELD_HEIGHT;
  ...
  if (metrics.statement) {
    ...
    metrics.width +=
      metrics.bayWidth +
      4 * Blockly.BlockSvg.CORNER_RADIUS +
...
Blockly.BlockSvg.prototype.renderDraw_ = function (metrics) {
  ...
  if (metrics.imageField) {
    ...
    // block.
    var imageFieldX =
      metrics.width - imageFieldSize.width - Blockly.BlockSvg.SEP_SPACE_X / 1.5;
    var imageFieldY =
      metrics.height - imageFieldSize.height - Blockly.BlockSvg.SEP_SPACE_Y;
    if (metrics.endCap) {
  ...
  if (this.getFieldShadowBlock_()) {
    ...
    var valueX =
      ...
      (metrics.bayWidth
        ? 2 * Blockly.BlockSvg.GRID_UNIT + Blockly.BlockSvg.NOTCH_WIDTH * 2
        : 0) +
      metrics.bayWidth;
    if (metrics.startHat) {
 ...
 * @private
 */
Blockly.BlockSvg.prototype.renderDrawLeft_ = function (
  steps,
  connectionsXY,
  metrics
) {
  ...
  if (metrics.startHat) {
    ...
    var connectionY =
      connectionsXY.y + metrics.height - Blockly.BlockSvg.CORNER_RADIUS * 2;
...
Blockly.BlockSvg.prototype.positionNewBlock = function (
  ...
  if (newConnection.type == Blockly.NEXT_STATEMENT) {
    ...
    if (newConnection == newBlock.getFirstStatementConnection()) {
      dy -=
        existingConnection.sourceBlock_.getHeightWidth(true).height -
