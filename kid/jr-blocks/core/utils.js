...
/**
 * 获取明文 (msg/message.js)
 * 要允许ADVANCED_OPTIMIZATIONS，不能将variable.name和variable ['name']组合在一起。 要访问导出的Blockly.Msg.Something，需要通过导出的确切名称进行访问。 请注意，所有导出操作都是在生成的js文件中最后发生的，因此在加载JavaScript之前将无法访问它们!
 * @return {!Object.<string, string>} The message array.
 * @private
 */
Blockly.utils.getMessageArray_ = function () {
  // 返回了 Msg文件夹下的message.js
  return goog.global["Blockly"]["Msg"];
};

/**
 * 移除元素属性。与Element.removeAttribute()类似，但它适用于IE 10中的SVG元素
 * @param {!Element} element DOM元素，用于从中删除属性.
 * @param {string} attributeName 要删除的属性名称.
 */
Blockly.utils.removeAttribute = function (element, attributeName) {
  // goog.userAgent.isVersion已过时, but the replacement is goog.userAgent.isVersionOrHigher.
...
/**
 * 将CSS类添加到元素.与Closure的goog.dom.classes.add类似，但它处理SVG元素
 * @param {!Element} element 需添加样式的元素.
 * @param {string} className 样式类名.
 * @return {boolean} True 添加成功, false已存在.
 */
...
/**
 * 为元素移除样式.
 * @param {!Element} element 需移除样式的元素.
 * @param {string} className 样式类名.
 * @return {boolean} True 移除成功, false不存在.
 */
...
/**
 * 获取相对坐标.返回此元素相对于其父元素的左上角的坐标。仅适用于SVG元素和子元素（例如rect，g，path）.
 * @param {!Element} element SVG元素查找的坐标.
 * @return {!goog.math.Coordinate} 具有.x和.y属性的对象.
 */
Blockly.utils.getRelativeXY = function (element) {
  ...
  // 然后检查样式=转换：translate（...）或translate3d（...）
