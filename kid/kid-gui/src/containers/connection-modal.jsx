/*
 * @Author: Satya
 * @Date: 2020-08-02 18:48:43
 * @Last Modified by: Satya
 * @Last Modified time: 2020-12-12 16:11:27
 * doc:
 *  更换扩展库预览文件在本文件的使用
 */
...
class ConnectionModal extends React.Component {
  constructor(props) {
    ...
    this.state = {
      extension: EXTENSIONLIBRARY_DATA().find(
        (ext) => ext.extensionId === props.extensionId
      ),
      phase: VM.getPeripheralIsConnected(props.extensionId)
        ? PHASES.connected
        : PHASES.scanning,
    };
  }
  componentDidMount() {
    VM.on("PERIPHERAL_CONNECTED", this.handleConnected);
    VM.on("PERIPHERAL_REQUEST_ERROR", this.handleError);
  }
  componentWillUnmount() {
    VM.removeListener("PERIPHERAL_CONNECTED", this.handleConnected);
    VM.removeListener("PERIPHERAL_REQUEST_ERROR", this.handleError);
  }
  handleScanning() {
    this.setState({
  ...
  handleConnecting(peripheralId) {
    VM.connectPeripheral(this.props.extensionId, peripheralId);
    this.setState({
  ...
  handleDisconnect() {
    try {
      VM.disconnectPeripheral(this.props.extensionId);
    } finally {
      this.props.onCancel();
    }
  }
  handleCancel() {
    try {
      ...
      if (!VM.getPeripheralIsConnected(this.props.extensionId)) {
        VM.disconnectPeripheral(this.props.extensionId);
      }
    } finally {
      // Close the modal.
