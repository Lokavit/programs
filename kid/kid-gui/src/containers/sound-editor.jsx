...
class SoundEditor extends React.Component {
  ...
  submitNewSamples(samples, sampleRate, skipUndo) {
    ...
    try {
      ...
      if (wavBuffer.byteLength > SOUND_BYTE_LIMIT) {
        ...
        wavBuffer = null;
        console.error(
          `Refusing to encode sound larger than ${SOUND_BYTE_LIMIT} bytes`
        );
      }
    } catch (e) {
      ...
      console.error(
        `Encountered error while trying to encode sound update: ${e}`
      );
    }
  ...
  handleChangeName(name) {
    VM.renameSound(this.props.soundIndex, name);
  }
  handleDelete() {
    ...
    effects.process((renderedBuffer, adjustedTrimStart, adjustedTrimEnd) => {
      ...
      if (success) {
        if (this.state.trimStart === null) {
          ...
          this.setState(
            { trimStart: adjustedTrimStart, trimEnd: adjustedTrimEnd },
            this.handlePlay
          );
        }
      }
