/*
 * @Author: Satya
 * @Date: 2020-11-25 15:08:52
 * @Last Modified by: Satya
 * @Last Modified time: 2020-12-15 14:39:41
 * doc: 该文件中工具为kid-svg-renderer使用
 * 暂时放在这里，之后整合 utility.js中
 *  SvGElement 相关
 *  转换2.0-3.0的笔画宽度
 */
...
class SvgElement {
  ...
  static attributeNamespace() {
    return {
      ...
      // 设置“ xmlns：xlink”时，IE需要xmlns命名空间. See #984
...
/**
 * @module 转换2.0-3.0的笔画宽度
 */
/**
 * @fileOverview 应用转换以匹配2.0和3.0中的笔划宽度
 */

// 改编自paper.js的Path.applyTransform
const _parseTransform = function (domElement) {
  let matrix = TransformationMatrix.identity();
  // let matrix = compose(translate(40, 40), rotate(Math.PI / 2), scale(2, 4));
  const string =
  ...
  for (const transform of transforms) {
    ...
    switch (command) {
      case "matrix":
        matrix = TransformationMatrix.compose(matrix, {
          a: v[0],
          b: v[1],
          c: v[2],
          d: v[3],
        ...
        break;
      case "rotate":
        matrix = TransformationMatrix.compose(
          matrix,
          TransformationMatrix.rotateDEG(v[0], v[1] || 0, v[2] || 0)
        );
        break;
      case "translate":
        matrix = TransformationMatrix.compose(
          matrix,
          TransformationMatrix.translate(v[0], v[1] || 0)
        );
        break;
      case "scale":
        matrix = TransformationMatrix.compose(
          matrix,
          TransformationMatrix.scale(v[0], v[1] || v[0])
        );
        break;
      case "skewX":
        matrix = TransformationMatrix.compose(
          matrix,
          TransformationMatrix.skewDEG(v[0], 0)
        );
        break;
      case "skewY":
        matrix = TransformationMatrix.compose(
          matrix,
          TransformationMatrix.skewDEG(0, v[0])
        );
        break;
      default:
...
// 如果矩阵不可逆，则返回null。 否则返回给定的椭圆（由transform变换），一个对象{radiusX，radiusY，rotation}.
const _calculateTransformedEllipse = function (
  ...
  const c = -transform.b;
  const d = transform.d;
  // 由于其他参数决定了SVG中椭圆的平移，因此我们不必担心e和f是什么。.
  const det = a * d - b * c;
  // Non-invertible matrix
  if (det === 0) return null;

  // rotA，rotB和rotC表示旋转椭圆公式的Ax ^ 2 + Bxy + Cy ^ 2 = 1系数
  ...
  // 计算变换椭圆的椭圆公式A，B和C表示变换椭圆公式中的Ax ^ 2 + Bxy + Cy ^ 2 = 1 / det / det系数，按反det平方进行缩放（以保持精度）
...
const _transformPath = function (pathString, transform) {
  if (
    !transform ||
    TransformationMatrix.toString(transform) ===
      TransformationMatrix.toString(TransformationMatrix.identity())
  )
    return pathString;
  // First split the path data into parts of command-coordinates pairs
  // Commands are any of these characters: mzlhvcsqta
  const parts = pathString && pathString.match(/[mlhvcsqtaz][^mlhvcsqtaz]*/gi);
  let coords;
  let relative = false;
  let previous;
  let control;
  ...
  const getString = function (point) {
    const transformed = TransformationMatrix.applyToPoint(transform, point);
    ...
    switch (lower) {
      ...
      case "c":
        ...
        for (let j = 0; j < length; j += 6) {
          ...
          result += `C ${getString(handle1)}${getString(control)}${getString(
            current
          )}`;
        }
        break;
...
const _isGraphicsElement = function (element) {
  return (
    element.tagName && GRAPHICS_ELEMENTS.includes(element.tagName.toLowerCase())
  );
};
...
const _createGradient = function (gradientId, svgTag, bbox, matrix) {
  ...
  const getValue = function (
    ...
    // 将值解释为数字。 从不返回NaN，而是0.
    // 如果该值是一个数字序列，则parseFloat将返回第一个出现的数字，对于现在.
  ...
  let matrixString = TransformationMatrix.toString(matrix);
  ...
  if (scaleToBounds) {
    const boundsMatrix = TransformationMatrix.compose(
      TransformationMatrix.translate(bbox.x, bbox.y),
      TransformationMatrix.scale(bbox.width, bbox.height)
    );
    origin = TransformationMatrix.applyToPoint(boundsMatrix, origin);
    if (destination)
      destination = TransformationMatrix.applyToPoint(
        boundsMatrix,
        destination
      );
    if (radius) {
    ...
    if (focal) focal = TransformationMatrix.applyToPoint(boundsMatrix, focal);
  }

  if (radial) {
    origin = TransformationMatrix.applyToPoint(matrix, origin);
    const matrixScale = _getScaleFactor(matrix);
    radius = _quadraticMean(matrixScale.x, matrixScale.y) * radius;
    if (focal) focal = TransformationMatrix.applyToPoint(matrix, focal);
  } else {
    ...
    // Transform points
    gradientPerpendicular = TransformationMatrix.applyToPoint(
      matrix,
      gradientPerpendicular
    );
    origin = TransformationMatrix.applyToPoint(matrix, origin);
    destination = TransformationMatrix.applyToPoint(matrix, destination);
...
const transformStrokeWidths = function (svgTag, windowRef, bboxForTesting) {
  const inherited = TransformationMatrix.identity();
    ...
    if (_isContainerElement(element)) {
      ...
      for (let i = 0; i < element.childNodes.length; i++) {
        applyTransforms(
          ...
          TransformationMatrix.compose(matrix, _parseTransform(element)),
          strokeWidth,
          fill,
          stroke
        );
      ...
      if (element.attributes.fill) fill = element.attributes.fill.value;
      if (element.attributes.stroke) stroke = element.attributes.stroke.value;
      matrix = TransformationMatrix.compose(matrix, _parseTransform(element));
      if (
        TransformationMatrix.toString(matrix) ===
        TransformationMatrix.toString(TransformationMatrix.identity())
      ) {
        element.removeAttribute("transform");
      ...
      if (fillGradientId || strokeGradientId) {
        ...
        if (bboxForTesting) {
          ...
          try {
            ...
            const svg = SvgElement.set(
              doc.createElementNS(SvgElement.svg, "svg")
            );
            const path = SvgElement.set(
              doc.createElementNS(SvgElement.svg, "path")
            );
