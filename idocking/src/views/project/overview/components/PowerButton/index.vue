 ...
 * @LastEditTime: 2019-10-30 15:37:18
 -->
<template>
...
export default {
  ...
  methods: {
    handleCommand(command) {
      // 所有formMod保存后才能提交
      if(this._projectOverview._canNext()) {
        if(command === 'SUBMIT') {
          this.submitIt()
    ...
    approveIt(dialog, formMod) {
      ...
      approveProjet(this.projectId, comment).then(res => {
        this.$message({showClose: true,message: this.$t('common.doOk'),type: 'success'})
        this.getOperation()
        this.$emit('state-change')
    ...
    rejectIt(dialog, formMod) {
      ...
      rejectProjet(this.projectId, comment).then(res => {
        this.$message({showClose: true,message: this.$t('common.doOk'),type: 'success'})
        this.$emit('state-change')
        this.$refs[dialog].close()
      })
    ...
    getOperation() {
      getOperation(this.projectId).then(res => {
        // this.power = res.data.filter(item => { return item !== 'MODIFY' })

        this.power = res.data
        this._projectOverview.powerBtns = res.data
      })
    }
  },
  created() {
