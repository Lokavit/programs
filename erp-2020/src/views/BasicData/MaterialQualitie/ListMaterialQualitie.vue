<template>
  <section>
    ...
    <el-form :model="filterCondition" size="mini">
      <div class="form_container">
        <div class="form_row">
          <!-- 第一列 -->
          <aside>
            <KftLabel label="材质类别"></KftLabel>
            <el-input v-model="filterCondition.mqCode" placeholder="请输入材质类别" @keyup.enter.native="onFilter" clearable @clear="onFilter" size="mini"></el-input>
          </aside>
          <!-- 第二列 -->
          <aside>
            <KftLabel label="材质代码"></KftLabel>
            <el-input v-model="filterCondition.mqName" placeholder="请输入材质代码" @keyup.enter.native="onFilter" clearable @clear="onFilter" size="mini"></el-input>
          </aside>
          <!-- 状态   -->
          <aside>
    ...
    <el-table v-loading="tableLoading" :data="tableData.content" highlight-current-row size="mini" :header-cell-style="GLOBAL.LIST_TABLE_HEADER_CELL_STYLE" :cell-style="GLOBAL.LIST_TABLE_CELL_STYLE" stripe @current-change="currentSelectRow">
      ...
      <el-table-column prop="mqCode" label="材质类别" show-overflow-tooltip></el-table-column>
      <el-table-column prop="mqName" label="材质代码" show-overflow-tooltip></el-table-column>
...
<script>
import { getMaterialQualitie } from "@/api/basicdata";
export default {
  /** 基础数据 材质代码 列表 */
  ...
  methods: {
    ...
    onCreate(event) {
      ...
      let isCreateForm = this.$store.getters.taskGroup.find(
        task => task.path == `/basicdata/material_qualitie-form/create`
      );
      // 如果已经有新增表单
      if (isCreateForm) {
        ...
        this.createTask(
          {
            path: "/basicdata/material_qualitie-form/",
    ...
    onDetail() {
      if (!this.currentRow) {
        ...
        this.beforeClickDetailEvent(this.currentRow.mqCode);
      }
    },
    ...
    beforeClickDetailEvent(code) {
      ...
      let tempTask = this.$store.getters.taskGroup.find(
        task => task.path == `/basicdata/material_qualitie-form/${code}`
      );
      if (tempTask) {
        // 如果找到任务项
    ...
    async getDataList(page, size, filterCondition = {}) {
      ...
      try {
        ...
        this.tableData = await getMaterialQualitie(params);
