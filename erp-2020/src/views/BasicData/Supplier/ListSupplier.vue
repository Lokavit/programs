<template>
  <section>
    ...
    <el-form :model="filterCondition" size="mini">
      <div class="form_container">
        <div class="form_row">
          <!-- 编码 -->
          <aside>
            <KftLabel label="编码"></KftLabel>
            <el-input v-model="filterCondition.supplierCode" placeholder="请输入编码" @keyup.enter.native="onFilter" clearable @clear="onFilter" size="mini"></el-input>
          </aside>
          <!-- 名称 -->
          <aside>
            <KftLabel label="名称"></KftLabel>
            <el-input v-model="filterCondition.supplierName" placeholder="请输入名称" @keyup.enter.native="onFilter" clearable @clear="onFilter" size="mini"></el-input>
          </aside>
          <!-- 主联系人  -->
          <aside>
            <KftLabel label="主联系人"></KftLabel>
            <el-input v-model="filterCondition.contactName" placeholder="请输入主联系人" @keyup.enter.native="onFilter" clearable @clear="onFilter" size="mini"></el-input>
          </aside>
          <!-- 状态   -->
          <aside>
    ...
    <el-table v-loading="tableLoading" :data="tableData.content" highlight-current-row size="mini" :header-cell-style="GLOBAL.LIST_TABLE_HEADER_CELL_STYLE" :cell-style="GLOBAL.LIST_TABLE_CELL_STYLE" stripe @current-change="currentSelectRow">
      ...
      <el-table-column prop="supplierCode" label="编码" show-overflow-tooltip></el-table-column>
      <el-table-column prop="supplierName" label="名称" show-overflow-tooltip></el-table-column>
      <el-table-column prop="contactName" label="主联系人" show-overflow-tooltip></el-table-column>
      <el-table-column prop="contactTel" label="联系方式" show-overflow-tooltip></el-table-column>
...
<script>
import { getSupplier } from "@/api/basicdata";
export default {
  /** 基础数据 供应商 列表 */
  name: "ListSupplier",
  data() {
  ...
  methods: {
    ...
    onCreate(event) {
      ...
      let isCreateForm = this.$store.getters.taskGroup.find(
        task => task.path == `/basicdata/supplier-form/create`
      );
      // 如果已经有新增表单
      if (isCreateForm) {
    ...
    onDetail() {
      if (!this.currentRow) {
        ...
        this.beforeClickDetailEvent(this.currentRow.supplierCode);
      }
    },
    ...
    beforeClickDetailEvent(code) {
      ...
      let tempTask = this.$store.getters.taskGroup.find(
        task => task.path == `/basicdata/supplier-form/${code}`
      );
      if (tempTask) {
        // 如果找到任务项
    ...
    async getDataList(page, size, filterCondition = {}) {
      ...
      try {
        ...
        this.tableData = await getSupplier(params);
