<template>
  <section>
      ...
      <div class="form_container">
        <div class="form_row">
          ...
          <aside>
            ...
            <el-input v-model="filterCondition.whsCode" placeholder="请输入编码" @keyup.enter.native="onFilter" clearable @clear="onFilter" size="mini"></el-input>
          </aside>
          <!-- 名称 -->
          <aside>
            <KftLabel label="名称"></KftLabel>
            <el-input v-model="filterCondition.whsName" placeholder="请输入名称" @keyup.enter.native="onFilter" clearable @clear="onFilter" size="mini"></el-input>
          </aside>
          <!-- 状态   -->
          <aside>
    ...
    <el-table v-loading="tableLoading" :data="tableData.content" highlight-current-row size="mini" :header-cell-style="GLOBAL.LIST_TABLE_HEADER_CELL_STYLE" :cell-style="GLOBAL.LIST_TABLE_CELL_STYLE" stripe @current-change="currentSelectRow">
      ...
      <el-table-column prop="whsCode" label="编码" show-overflow-tooltip></el-table-column>
      <el-table-column prop="whsName" label="名称" show-overflow-tooltip></el-table-column>
      <el-table-column prop="type" label="分类" show-overflow-tooltip></el-table-column>
      <el-table-column prop="contactName" label="保管员" show-overflow-tooltip></el-table-column>
...
<script>
import { getWarehouse } from "@/api/basicdata";
export default {
  /** 基础数据 仓库 列表 */
  name: "ListWarehouse",
  data() {
  ...
  methods: {
    ...
    onCreate(event) {
      ...
      let isCreateForm = this.$store.getters.taskGroup.find(
        task => task.path == `/basicdata/warehouse-form/create`
      );
      // 如果已经有新增表单
      if (isCreateForm) {
    ...
    onDetail() {
      if (!this.currentRow) {
        ...
        this.beforeClickDetailEvent(this.currentRow.whsCode);
      }
    },
    ...
    beforeClickDetailEvent(code) {
      ...
      let tempTask = this.$store.getters.taskGroup.find(
        task => task.path == `/basicdata/warehouse-form/${code}`
      );
      if (tempTask) {
        // 如果找到任务项
    ...
    async getDataList(page, size, filterCondition = {}) {
      ...
      try {
        ...
        this.tableData = await getWarehouse(params);
      } catch (err) {}
      /** 列表数据加载效果：关闭 */
