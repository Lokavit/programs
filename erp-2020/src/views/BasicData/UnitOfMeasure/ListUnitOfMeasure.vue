<template>
  <section>
      ...
      <div class="form_container">
        <div class="form_row">
          ...
          <aside>
            ...
            <el-input v-model="filterCondition.uomCode" placeholder="请输入编码" @keyup.enter.native="onFilter" clearable @clear="onFilter" size="mini"></el-input>
          </aside>
          <!-- 第二列 -->
          <aside>
            <KftLabel label="名称"></KftLabel>
            <el-input v-model="filterCondition.uomName" placeholder="请输入名称" @keyup.enter.native="onFilter" clearable @clear="onFilter" size="mini"></el-input>
          </aside>
          <!-- 状态   -->
          <aside>
    ...
    <el-table v-loading="tableLoading" :data="tableData.content" highlight-current-row size="mini" :header-cell-style="GLOBAL.LIST_TABLE_HEADER_CELL_STYLE" :cell-style="GLOBAL.LIST_TABLE_CELL_STYLE" stripe @current-change="currentSelectRow">
      ...
      <el-table-column prop="uomCode" label="编码" show-overflow-tooltip></el-table-column>
      <el-table-column prop="uomName" label="名称" show-overflow-tooltip></el-table-column>
      <el-table-column prop="accuracy" label="精度" show-overflow-tooltip></el-table-column>
      <el-table-column prop="roundType" label="舍入类型" show-overflow-tooltip>
        <template v-slot="scope">
          <!-- 于结构元素内直接使用，无需加this. -->
          <span>{{GLOBAL.returnListUOMRoundType(scope.row.roundType).label}}</span>
        </template>
      </el-table-column>
...
<script>
import { getUOM } from "@/api/basicdata";
export default {
  /** 基本数据 计量单位 列表 */
  ...
  methods: {
    ...
    onCreate(event) {
      ...
      let isCreateForm = this.$store.getters.taskGroup.find(
        task => task.path == `/basicdata/uom-form/create`
      );
      // 如果已经有新增表单
      if (isCreateForm) {
    ...
    onDetail() {
      if (!this.currentRow) {
        ...
        this.beforeClickDetailEvent(this.currentRow.uomCode);
      }
    },
    ...
    beforeClickDetailEvent(code) {
      ...
      let tempTask = this.$store.getters.taskGroup.find(
        task => task.path == `/basicdata/uom-form/${code}`
      );
      if (tempTask) {
        // 如果找到任务项
    ...
    async getDataList(page, size, filterCondition = {}) {
      ...
      try {
        ...
        this.tableData = await getUOM(params);
      } catch (err) {}
      /** 列表数据加载效果：关闭 */
