...
import FormPurchaseReturn from "./FormPurchaseReturn";
export default {
  // 采购模块 采购退货单 列表页
  name: "ListPurchaseReturn",
  components: { FormPurchaseReturn },
  data() {
    return {
      /** 列表中的表头样式 */
  ...
  computed: {
    ...
    /** 当前窗体标注组 [后台获取所有标注] */
    labels() {
      return this.$store.getters.labels.PurchaseReturn.listLbales;
    }
  },
  watch: {},
  created() {},
  ...
  methods: {
    ...
    onCell(event) {
      event.cancelBubble = true; // 取消父组件点击事件 [事件冒泡]
      this.openModal(event.target.innerText); // 调用打开模态框函数，将获取到的单元格数据[itemCode]物料编码传入
    },
    ...
    async getDataList(page, size, search = {}) {
      ...
      try {
        ...
        let res = await getPurchaseReturns(params);
        this.tableData = res;
      } catch (err) {}
      /** 列表数据加载效果：关闭 */
