<template>
  <section>
    ...
    <el-table v-loading="tableLoading" :data="tableData.content" highlight-current-row size="mini" :header-cell-style="GLOBAL.LIST_TABLE_HEADER_CELL_STYLE" :cell-style="GLOBAL.LIST_TABLE_CELL_STYLE" stripe @current-change="currentSelectRow">
      ...
      <el-table-column prop="itemGroupName" label="规格" show-overflow-tooltip></el-table-column>
...
import { postActivitieimport, getProjectDownload } from "@/api/manufacture";
export default {
  /** 钢材主数据 列表页 */
  name: "ListSteel",
  data() {
  ...
  methods: {
    ...
    onCreate(event) {
      ...
      let isCreateForm = this.$store.getters.taskGroup.find(
        task => task.path == `/mm/material/steel-form/create`
      );
      // 如果已经有新增表单
      if (isCreateForm) {
    ...
    beforeClickDetailEvent(code) {
      ...
      let tempTask = this.$store.getters.taskGroup.find(
        task => task.path == `/mm/material/steel-form/${code}`
      );
      if (tempTask) {
        // 如果找到任务项
