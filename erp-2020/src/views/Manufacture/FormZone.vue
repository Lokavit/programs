    ...
    <el-form :model="form" size="mini" :rules="rules" ref="form">
      ...
      <div class="form_container">
        ...
        <div class="form_row">
          <!-- 类型 -->
          <aside>
            <KftLabel :label="labels.kind.value|FormatLabelSuffix" :isRequired="true"></KftLabel>
            <el-select v-model="form.kind" :placeholder="`请选择${labels.kind.value}`" @change="changeKind" size="mini">
              <el-option v-for="(item,index) in kindOptions" :key="item.index" :label="item.name" :value="item.name">
...
<script>
import { getZoneInfo, postZone, putZone } from "@/api/manufacture";
export default {
  // 生成模块 区域登记 表单页
  name: "FormZone",
  data() {
    return {
      ...
      // 当前表单页，类型的下拉选项组
      kindOptions: [
        { name: "zone", description: "区域" },
        { name: "system", description: "系统" }
      ], // 类型选择
      form: {
        kind: "zone", // 类型下拉选项的默认值
  ...
  computed: {
    ...
    /** 当前表单的所有标注 */
    labels() {
      return this.$store.getters.labels.Zone.formLabels;
    },
    /** 全局窗体组 */
    modalGroup() {
  ...
  mounted() {
    ...
    if (!this.passValue.entry) {
      ...
      console.log("passValue空对象，根节点新增：", this.passValue);
    }
    ...
    if (this.passValue.status == "modifty") {
      // console.log("STATUS:", this.passValue.status);
  ...
  methods: {
    ...
    async postData() {
      ...
      try {
        const res = await postZone(this.form);
        this.form = res; // 返回结果中的数据，回显至表单上
        console.log("回显RES:", res);
        this.form.kind = res.kind; // 类型下拉选项值的回显
        this.form.assemblyIndicator = res.assemblyIndicator.toString();
        this.form.cuttingIndicator = res.cuttingIndicator.toString();
        this.form.erectionIndicator = res.erectionIndicator.toString();
        this.form.steelPOIndicator = res.steelPOIndicator.toString();
    ...
    async putData() {
      ...
      this.form = {
        assemblyIndicator: stringToBoolean(this.form.assemblyIndicator),
        cuttingIndicator: stringToBoolean(this.form.cuttingIndicator),
        erectionIndicator: stringToBoolean(this.form.erectionIndicator),
        steelPOIndicator: stringToBoolean(this.form.steelPOIndicator),
        ...formData
      };
      // console.log("FORM:", this.form);
      try {
        const res = await putZone(this.form);
        /** 在修改某节点的情况下,
         * 将请求数据赋值给当前表单,并且对几个单选框进行赋值。
         */
        this.form = res;
