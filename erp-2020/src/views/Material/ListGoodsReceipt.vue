...
<script>
import { getGoodsReceipt } from "@/api/stock";
import FormGoodsReceipt from "./FormGoodsReceipt";
export default {
  // 库存模块 库存收货单 列表页
  name: "ListGoodsReceipt",
  components: { FormGoodsReceipt },
  data() {
    return {
      /** 列表中的表头样式 */
  ...
  methods: {
    ...
    async getDataList(page, size, search = {}) {
      ...
      try {
        ...
        let res = await getGoodsReceipt(params);
        this.tableData = res;
      } catch (err) {}
      /** 列表数据加载效果：关闭 */
