<template>
  <section :style="{'width':width}" v-loading="formLoading">
    <el-form :model="form" size="mini" :rules="rules" ref="form">
      ...
      <!-- 编辑表格区域  :span-method="objectSpanMethod" 该函数为行合并 -->
      <el-table :data="tableData" border highlight-current-row size="mini" :header-cell-style="headerCellStyle" :cell-style="cellStyle" :style="[styleObject]" :max-height="tableMaxHeight" :row-style="rowStyle" :row-class-name="rowClassName" @current-change="handleCurrentRow">
        <el-table-column type="index" label="#"></el-table-column>
        <el-table-column label="物料编码" min-width="12%" prop="itemCode">
        ...
        <el-table-column label="数量" prop="quantity" min-width="8%">
          <template v-slot="scope">
            <small>
              <!-- <el-input-number size="mini" v-model="scope.row.quantity" :min="1"></el-input-number> -->
              <el-input size="mini" v-model="scope.row.quantity" :placeholder="`0`"></el-input>
            </small>
            <span>{{scope.row.quantity}}</span>
          </template>
        </el-table-column>

        <el-table-column prop="managementType" label="物料管理方式" min-width="10%">
          <template v-slot="scope">{{scope.row.managementType|FormatManagementType}}</template>
        </el-table-column>

        <el-table-column label="仓库编码" prop="whsCode" min-width="12%">
          <template v-slot="scope">
            <small>
        ...
        <!-- td内嵌多行 -->
        <el-table-column label="创建批次">
          <el-table-column label="编码" prop="batchNumber" min-width="10%">
            <template v-slot="scope">
              ...
              <small>
                <el-input size="mini" v-model="scope.row.batchNumber" :placeholder="`0`" @keyup.enter.native="onCreate($event,scope.$index)"></el-input>
              </small>
              <span>{{scope.row.batchNumber}}</span>
            </template>
          </el-table-column>

          <el-table-column label="数量" prop="batchQuantity" min-width="8%">
            <template v-slot="scope">
              <template v-if="isBatch">
                <span>{{scope.row.batchNumber}}</span>
                <hr />
              ...
              <small>
                <el-input size="mini" v-model="scope.row.batchQuantity" :placeholder="`0`"></el-input>
              </small>
              <span>{{scope.row.batchQuantity}}</span>
            </template>
          </el-table-column>
...
export default {
  ...
  components: { BatchCreateDialog, InventoryDialog },
  data() {
    return {
  ...
  methods: {
    /** 创建一个 */
    onCreate(event, index) {
      console.log("创建一个", event, "INDEX:", index);
      this.isBatch = true;
    },
    ...
    onSubmit() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          /*
            // 返回需要批次管理类型的数据集
            let lines_BATCHNUMBER = this.GLOBAL.returnDataByType(
              "BATCHNUMBER",
              this.removeEmptyData(this.tableData)
            );
            console.log("返回批次管理的数据:", lines_BATCHNUMBER);
            // 返回需要序列号管理类型的数据集
            let lins_SERIALNUMBER = this.GLOBAL.returnDataByType(
              "SERIALNUMBER",
              this.removeEmptyData(this.tableData)
            );
            console.log("返回序列号管理的数据：", lins_SERIALNUMBER);
          */
          ...
          if (needBatchLines == undefined || needBatchLines.length <= 0) {
            this.postData(); // 调用新增数据函数
          } else if (needBatchLines != undefined || needBatchLines.length > 0) {
            /**
             * 如果有需要批次的数据  ， 分为以下几种情况
            ...
            for (let i = 0; i < needBatchLines.length; i++) {
              ...
              if (needBatchLines[i].batches == undefined) {
                 ...
                 */
                this.$set(needBatchLines[i], "batches", this.tData[i].batches);
    ...
    async getDataInfo() {
      try {
        /** 窗体内容加载效果：开启 */
        this.formLoading = true;
        const res = await getGoodsReceiptInfo(this.passValue);
        this.form = res;
    ...
    async postData() {
      try {
        ...
        const res = await postGoodsReceipt(this.form);
